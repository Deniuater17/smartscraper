...
"Tecno Spark 10 Pro",
"pantalla rota",
    ]); // Lista estática de colecciones raíz
    ...
    const handleCollectionSelect = async (collectionName) => {
        ...
        try {
            const data = await fetchProductsFromCollection(`"${collectionName}"`); // Usa comillas para que coincida
            setProducts(data);
        } catch (error) {
        ...
        <div>
            ...
            <div style={{ display: "flex" }}>
                ...
                <div style={{ flex: 3, paddingLeft: "20px" }}>
                    {selectedCollection && (
                        <>
                            <h2>Productos en la colección "{selectedCollection}":</h2>
                            {products.length > 0 ? (
                                <table border="1">
                                    <thead>
                                        <tr>
